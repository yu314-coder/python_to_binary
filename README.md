...
- A fixed table of 40 exported CPython entry points: `Py_Initialize`,
...
| py2bin compiles C that calls the CPython C-API, without gcc | Yes, for a vetted subset on `darwin-arm64` | py2bin's own C parser and ARM64 encoder produce a Mach-O that dyld-binds the interpreter and calls 40 vetted entry points. Verified by running the binary and diffing stdout and exit status against the same program under CPython. |
...
| `sys.argv` | Restricted, POSIX only | `len(sys.argv)` and `sys.argv[i]`, each element copied out of the kernel's C string into a native string. `sys.argv[0]` is the binary's own path. macOS passes them in registers and Linux on the stack; Windows is refused, since it would need GetCommandLineW |
