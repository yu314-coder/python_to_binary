...
- A fixed table of 65 exported CPython entry points: `Py_Initialize`,
  ...
  `PyBytes_FromStringAndSize`, `PyNumber_Negative`,
  `PyNumber_Positive`, `PyNumber_Invert`,
...
| py2bin compiles C that calls the CPython C-API, without gcc | Yes, for a vetted subset on `darwin-arm64` | py2bin's own C parser and ARM64 encoder produce a Mach-O that dyld-binds the interpreter and calls 65 vetted entry points. Verified by running the binary and diffing stdout and exit status against the same program under CPython. |
